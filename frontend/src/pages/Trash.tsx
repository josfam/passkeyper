...
import { Lock, MoreHorizontal, Eye, EyeOff, ExternalLink, Trash2, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react";
...
const PasswordTrashPage: React.FC = () => {
  ...
  const [previewingPassword, setPreviewingPassword] = useState<PasswordEntry | null>(null);
  ...
  const handlePermanentDelete = async (password: PasswordEntry) => {
    try {
      await axios.delete(`http://127.0.0.1:5000/password/${password.id}/permanent`, {
        withCredentials: true
      });
      setTrashedPasswords(trashedPasswords.filter((p) => p.id !== password.id));
      setIsPreviewModalOpen(false);
      ...
      toast({
        title: "Delete Failed",
        description: "Failed to delete the password permanently. Please try again.",
        variant: "destructive"
      });
    }
  };
  ...
  const currentPageData = totalItems <= 7 ? filteredPasswords : filteredPasswords.slice(startIndex, endIndex);
        ...
        <TableBody>
          {currentPageData.map((password) => (
            ...
            >
              ...
              <TableCell>
                {password.moved_at ? new Date(password.moved_at).toLocaleDateString() : "Unknown"}
              </TableCell>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols items-center gap-4">
                <div>
                  ...
                  <Input id="name" name="name" value={previewingPassword.name} readOnly />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex">
                    <Input
                      ...
                      readOnly
                    />
                    {renderCopyButton(previewingPassword.username, "Username", copiedField, setCopiedField)}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex relative">
                    ...
                    {renderCopyButton(previewingPassword.password, "Password", copiedField, setCopiedField)}
                  </div>
                </div>
              </div>
              <div className="grid grid-cols items-center gap-4">
                <div>
                  ...
                  <div className="flex">
                    <Input id="url" name="url" value={previewingPassword.url} readOnly />
                    <Button
                      variant="ghost"
                      size="icon"
                      className="ml-2"
                      onClick={() => window.open(previewingPassword.url, "_blank")}
                    >
                      <ExternalLink className="h-4 w-4" />
                    </Button>
                    {renderCopyButton(previewingPassword.url, "URL", copiedField, setCopiedField)}
                  </div>
                </div>
              </div>
              <div>
                <Label htmlFor="notes">Notes</Label>
                <Textarea id="notes" name="notes" value={previewingPassword.notes} readOnly rows={4} />
              </div>
            </div>
          )}
